...
\newcommand{\stSmalltalkConceptsDefinition} {Smalltalk is an
  \emph{object oriented} language, \emph{dynamically typed}, with a
minimal syntax which can be learnt in \emph{fifteen minutes}.
Its main advantage comes from the fact that it is \emph{very coherent}:

\begin{itemize}
\item everything is an object: classes, methods, numbers, etc.
\item a really small number of rules and no exception.
\end{itemize}
...
All message sends return an object. All messages are sent to objects, called the message receiver. There exist three message types: unary, binary and keyword.}
...
The environment permits code evaluation by a simple keystroke,
immediately showing the result.  }
...
\newcommand{\stDynamicTypingDefinition} {Some languages force
  ...
  developer does not limit the variables to a given type.}
