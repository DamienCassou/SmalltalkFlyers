...
Sua maior vantagem vem do fato de ser \emph{muito consistente}:
...
acontece numa \emph{imagem} na qual todos os objetos vivem e são
modificados.}

\newcommand{\stSmalltalkSyntaxTerm}{A syntaxe do Smalltalk}
...
{Uma mensagem unária não tem argumentos.
...
\newcommand{\stBinaryMessagesDefinition}{Uma mensagem binária recebe apenas
  um argumento, é chamada por um símbolo e normalmente é usada para
...
{
  Uma mensagem de palavra-chave pode receber um ou mais argumentos. Os argumentos são inseridos entre cada palavra-chave, depois dos dois pontos.
...
\newcommand{\stDevelopmentEnvironmentDefinition} {A maioria das
  implementações de Smalltalk vem com um ambiente de desenvolvimento
  integrado que permite que você navegue no código-fonte e
  interaja com os objetos. Muitas ferramentas estão disponíveis, todas
  implementadas em Smalltalk graças a uma API de reflexão computacional:
...
O código pode ser inspecionado e avaliado diretamente na imagem, usando simples combinações de teclas e opções de menus.  }
...
\begin{description}
\item[Squeak:] implementação gratuita, de código aberto e
